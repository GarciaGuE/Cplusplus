...
class Decision{
	...
	public:
		...
		*/
		...
		/**
		 * Writes the object onto a stream
		 */
		...
		/**
		 * Initialise the object's variables from a stream
		 */
		...
		/**
		 * Compare if the object represents the same state as that passed by argument (board and decision)
		 */
		...
		/**
		 * Compare if the object considers the same board state as that passed by argument
		 */
...
/**
 * Read a set of Decision objects from a stream
 */
...
/**
 * Write a set of Decision objects onto a stream
 */
...
/**
 * Print a board state to the standard output
 */
...
/**
 * Return a set of possible decisions to be taken from a board state
 */
...
/**
 * Look for a decision (board + decision) in a set of decisions
 */
...
/**
 * Randomly select a number in [0, probabilities.size() - 1] according to the values in that vector. Larger values are selected with a higher probability
 */
...
/**
 * Compute a quality value for every possible alternatives.
 *
...
void getQualitiesOfAlternatives(vector<pair<int, int>> &alternatives, vector<double> &qualities, int** board,
	...
	for(auto alternative : alternatives){
		...
		//double probTie = (numTies + k_smoothing)/ (occurrences + k3_smoothing);
...
int main(int argc, char *argv[0]){
	 ...
	 */
	while((interactive==false && difftime(currentTime, initTime)<maxSeconds) || (interactive==true && iIterations<rounds)){
		...
		while(iLost==false && iWon==false && thereAreAlternatives){
			...
			//Compute the quality of each alternative
			getQualitiesOfAlternatives(alternatives, qualities, board, memory, ksmoothing_p1, k3smoothing_p1, 1);
			...
			if(interactive){
				...
				//Compute the quality of each alternative
				getQualitiesOfAlternatives(alternatives, qualities, board, memory, ksmoothing_p2, k3smoothing_p2, 2);
		...
		for(auto pair : memory){
			...
			if(pair.second){
				Decision* decision = pair.first;
				if(iWon){
					if(decision->numWins<MAXOCCURRENCES)
				 		decision->numWins++;
				 	else{
				 		int num=rand()%2;
				 		if(num==1 && numTies>MINOCCURRENCES)
				 			numTies--;
				 		else if(num==0 && decision->numLost>MINOCCURRENCES)
				 			decision->numLost--;
				 	}
				}
				else if(iLost){
				 	if(decision->numLost<MAXOCCURRENCES)
				 		decision->numLost++;
				 	else{
				 		int num=rand()%2;
				 		if(num==1 && numTies>MINOCCURRENCES)
				 			numTies--;
				 		else if(num==0 && decision->numWins>MINOCCURRENCES)
				 			decision->numWins--;
				 	}
				}
				else{
					if(decision->numTies<MAXOCCURRENCES)
						decision->numTies++;
					else{
						int num=rand()%2;
				 		if(num==1 && decision->numLost>MINOCCURRENCES)
				 			decision->numLost--;
				 		else if(num==0 && decision->numWins>MINOCCURRENCES)
				 			decision->numWins--;
					}
