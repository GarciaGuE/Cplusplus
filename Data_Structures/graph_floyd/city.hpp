...
struct City{
    ...
    City(const std::string& name_="", float latitude_=0.0f, float longitude_=0.0f) : name(name_), latitude(latitude_), longitude(longitude_){}
    bool operator<(const City& o) const{return name<o.name;}
    bool operator==(const City& o) const{return name==o.name;}
};
